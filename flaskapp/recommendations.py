...
ROW_MODEL_FILE = 'coe-solutions-215839/model/row.npy'
COL_MODEL_FILE = 'coe-solutions-215839/model/col.npy'
USER_MODEL_FILE = 'coe-solutions-215839/model/user.npy'
ITEM_MODEL_FILE = 'coe-solutions-215839/model/item.npy'
USER_ITEM_DATA_FILE = 'coe-solutions-215839/data/recommendation_events.csv'
...
class Recommendations(object):
    ...
    def __init__(self, local_model_path=LOCAL_MODEL_PATH):
        ...
        self._bucket = 'ml-engine-airflow-test'  # + project_id
    ...
    def _load_model(self, local_model_path):
        ...
        os.makedirs(os.path.join(local_model_path,
                                 'coe-solutions-215839/model'), exist_ok=True)
        os.makedirs(os.path.join(local_model_path,
                                 'coe-solutions-215839/data'), exist_ok=True)
        ...
        self.user_factor = np.load(os.path.join(
            local_model_path, ROW_MODEL_FILE), allow_pickle=True)
        self.item_factor = np.load(os.path.join(
            local_model_path, COL_MODEL_FILE), allow_pickle=True)
        self.user_map = np.load(os.path.join(
            local_model_path, USER_MODEL_FILE), allow_pickle=True)
        self.item_map = np.load(os.path.join(
            local_model_path, ITEM_MODEL_FILE), allow_pickle=True)
    ...
    def get_recommendations(self, user_id, num_recs):
        ...
        if user_idx:
            ...
            already_rated = self.user_items.get_group(user_id).userId
            ...
            article_recommendations = [self.item_map[i]
